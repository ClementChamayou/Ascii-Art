...
void SetPixel(Image* I,int i,int j,Pixel p)
{
	...
	if (I && i>=0 && i<I->w && j>=0 && j<I->h)
		I->dat[I->w*j+i] = p;
	else{
		printf("Issue in bmp.c : 66, SetPixel\n");
		exit(EXIT_FAILURE);
	}
}
...
Pixel GetPixel(Image* I,int i,int j)
{
	...
	if (I && i>=0 && i<I->w && j>=0 && j<I->h)
		...
		printf("Issue in bmp.c : 77, get pixel\n");
		exit(EXIT_FAILURE);
	}
}
...
Image* Charger(const char* fichier)
{
	...
	if (feof(F))
		printf("Il reste des octets à lire dans le fichier.\n");
	if (!F){
	...
	if (head.signature[0]!='B' || head.signature[1]!='M'){
        printf("Mauvaise signature ou BMP non supporté.\n");
		return NULL;  // mauvaise signature, ou BMP non support�.
    }
	if (head.imhead.bpp!=24){
        printf("Les pixels ne sont pas codés en 24 bits.\n");
